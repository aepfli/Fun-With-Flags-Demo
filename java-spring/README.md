...
The code lives on [`step/java-spring/7`](https://github.com/aepfli/Fun-With-Flags-Demo/tree/step/java-spring/7) — the handler reads a new `new_greeting_algo` flag, and the middleware passes `?userId=...` through as the OpenFeature `targetingKey` so the fractional rollout buckets stick per user.
...
- **`flags.json`** on `step/java-spring/7` defines `new_greeting_algo` with a flagd `fractional` rule, defaulting to 100% off. Bump the percentage and flagd hot-reloads — no app restart.
